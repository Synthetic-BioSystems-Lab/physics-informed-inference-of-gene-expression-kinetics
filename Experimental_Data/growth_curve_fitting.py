...
def gompertz_growth(t, x_0, k_g, K, t_lag):
    ...
    return np.where(
        ...
        K * (x_0 / K) ** np.exp(-k_g * (t - t_lag))  # Gompertz growth after t_lag
    )
...
for i in range(len(OD600_plots)):

    x_0_initial = cell_counts[OD600_plots[i]].iloc[0]
    p0 = [x_0_initial, 0.01, 0.5e9, 6]  
    bounds = ([0, 1e-10, 1e6, 0], [1e10, 1, 1e12, 12])

    popt, pcov = curve_fit(gompertz_growth, cell_counts['Kinetic read'], cell_counts[OD600_plots[i]], p0=p0, bounds=bounds)

    t = np.linspace(0, 20, 1000)
    y_fit = gompertz_growth(t, *popt)

    # Calculating R^2
    y_pred = gompertz_growth(cell_counts['Kinetic read'], *popt)
