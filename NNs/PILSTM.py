...
class PILSTM():
    ...
    def fit(self, X, Y, batch_size=32):
        ...
        mrna = torch.tensor(mrna, dtype=torch.float32).to(self.device)
        
        X = torch.tensor(X, dtype=torch.float32)
        Y = torch.tensor(Y, dtype=torch.float32)
        X = X.unsqueeze(-1)
        
        self.dataset = TensorDataset(X, Y, mrna)
        ...
        for self.epoch in range(self.n_epochs):
            ...
            for x_batch, y_batch, mrna_batch in train_loader:
                ...
                P = inv_minmax(x_batch, self.X_min, self.X_max)
                ...
                dP = np.array(dP)
                dP = torch.tensor(dP, dtype=torch.float32).to(self.device)
                P = torch.tensor(P, dtype=torch.float32).to(self.device)

                # ktl_b  = ktl.unsqueeze(1)
                # kdil_b = kdil.unsqueeze(1)
                # yfp_final = yfp_final.unsqueeze(1)

                # 0 = (ktl * M) - (kdil * A) - dAdt
                eps = 1e-6
                res = ktl @ mrna_batch - kdil @ P - (dP)
                scale1 = (ktl @ mrna_batch).abs() + (kdil @ P).abs() + dP.abs() + eps
            ...
            with torch.inference_mode():
                X_all, Y_all, mrna_all = self.dataset.tensors
